...
router.all('/paging', (req, res) => {
  ...
  if (req.body.notecategory == '全部') {
    var sql = 'select note.*,user.username,anthology.anthologyname from note,user,anthology where     note.anthologyid = anthology.anthologyid and anthology.userid = user.userid limit ' + start + '    ,7';
  } else {
