...
router.post('/getmessage', (req, res) => {
  res.header('Access-Control-Allow-Origin', '*');
  var userid = req.body.userid;
  todouserdata.GetMsg(userid, (err, results) => {
    if (err) {
      console.log(err);
      return;
    }
...
router.post('/getId', function (req, res) {
  ...
  todouserdata.getId(name, (err, result) => {
    if (err) {
      console.log(err);
      res.send(JSON.stringify({
        status: '102',
...
//判断一个是否关注
router.post('/getonemutual', (req, res)=>{
  res.header('Access-Control-Allow-Origin','*');
  console.log(req.body)
  const userid = req.body.userid,
        ouserid = req.body.ouserid;
todouserdata.GetOneMutual(userid, ouserid, (err, result)=>{
  if (err) {
    console.log(err);
    res.send(JSON.stringify({
      status:'102',
